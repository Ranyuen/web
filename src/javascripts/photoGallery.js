...
PhotoGallery.prototype.init = function (rootNode) {
  ...
  window.onscroll = throttle(this.onscroll.bind(this));
  this.applied();
};
...
PhotoGallery.prototype.loadNextpage = function () {
  var req, uri,
      _this = this;
  if (this._lockLoadingNextPage) {
    return;
  }
  this._lockLoadingNextPage = true;
  uri = URI('/photos/list.json').
  ...
  req.onreadystatechange = function () {
    ...
    if (req.status !== 200) {
      return console.error([req.status, req]);
    }
    try {
...
PhotoGallery.prototype.insertPhotoNodes = function (photos) {
  ...
  photos.forEach(function (photo) {
    ...
    photo.thumbUrl      = '/photos/show.jpg?id=' + photo.id + '&width=' + photo['thumb_width'];
