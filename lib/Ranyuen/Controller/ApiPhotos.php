...
class ApiPhotos implements ApiController
{
    public function render($method, array $uri_params, array $request_params)
    {
        ...
        $species_name = isset($request_params['species_name']) ? $request_params['species_name'] : null;
        $result = [];
        try {
            if ($species_name === null || $species_name === 'all') {
                ...
                $result = Photo::rawQuery(
                    ...
                    ['species_name' => '%' . strtolower($species_name) . '%', 'limit' => $limit, 'offset' => $offset]
                )->findMany();
            }
